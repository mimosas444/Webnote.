...
// ════════════════════════════════════════
//  DATETIME (hors popup uniquement)
// ════════════════════════════════════════
function updateDatetime() {
  ...
  ["auth-datetime","send-datetime"].forEach(id => {
    const el = $(id);
    if (el) { el.style.whiteSpace = "pre-line"; el.textContent = str; }
  });
}
updateDatetime();
...
document.querySelectorAll(".nav-tab, .mob-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    if (!currentUser && btn.dataset.page !== "landing") return;
...
async function loadDashboard() {
  ...
  replaceEl("share-ig",   b => b.addEventListener("click", () => { navigator.clipboard.writeText(currentShareLink).then(() => showToast("✅ Copié ! Colle dans ta bio Insta 📸")); }));
...
// ════════════════════════════════════════
//  COMMUNITY — visible à tous les connectés
// ════════════════════════════════════════
...
async function loadPolls() {
  ...
  try {
    // Tous les utilisateurs connectés voient les sondages actifs
...
function buildPollCard(pollId, data) {
  ...
  (data.options || []).forEach((opt, i) => {
    ...
    optionsHtml += `<div class="poll-option ${hasVoted?"voted":""}" data-poll="${pollId}" data-idx="${i}"><div class="poll-bar" style="width:${hasVoted?pct:0}%"></div><div class="poll-option-inner"><span class="poll-option-text">${escHtml(opt.text)}</span>${hasVoted?`<span class="poll-option-pct">${pct}%</span>`:""}</div></div>`;
  });
  ...
  if (!hasVoted && currentUser) {
    card.querySelectorAll(".poll-option").forEach(opt => opt.addEventListener("click", () => votePoll(pollId, parseInt(opt.dataset.idx), data)));
  }
  return card;
}
...
async function loadAdminPanel() {
  ...
  document.querySelectorAll(".emoji-opt").forEach(opt => {
    opt.addEventListener("click", () => {
      ...
      opt.classList.add("active"); selectedEmoji = opt.dataset.emoji;
    });
  });
  ...
  replaceEl("ann-submit", btn => btn?.addEventListener("click", async () => {
    ...
    try {
      ...
      showToast("📢 Communiqué publié !"); loadAdminAnnouncements();
  ...
  replaceEl("poll-submit", btn => btn?.addEventListener("click", async () => {
    ...
    try {
      await addDoc(collection(db,"polls"), { question, options: rawOptions.map(text => ({ text, votes: 0 })), voters: [], active: true, authorId: currentUser.uid, createdAt: serverTimestamp() });
      $("poll-question").value = ""; $("poll-options").value = "";
      showToast("📊 Sondage créé et visible par tous !"); loadAdminPolls();
    } catch (e) { showToast("Erreur."); }
...
async function loadAdminFeedback() {
  ...
  snap.forEach(d => {
    ...
    item.querySelector(".approve").addEventListener("click", async () => { await updateDoc(doc(db,"features",d.id), { approved: !data.approved }); showToast(data.approved?"Retrait effectué.":"✅ Approuvé !"); loadAdminFeedback(); });
...
function replaceEl(id, fn) {
  ...
  el.parentNode?.replaceChild(clone, el); fn(clone);
}
